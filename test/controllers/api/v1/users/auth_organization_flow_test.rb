...
class Api::V1::Users::AuthOrganizationFlowTest < ActionDispatch::IntegrationTest
  ...
  test "sign in authenticates via ldap when enabled" do
    ...
    authenticator = Struct.new(:result) { def call(login:, password:) = result }.new(auth_result)
    resolver = Struct.new(:user) { def call(email:, login:, name: nil) = user }.new(user)

    Auth::Ldap::Config.stub :current, ldap_config do
      Auth::Ldap::Authenticator.stub :new, authenticator do
        Auth::Ldap::UserResolver.stub :new, resolver do
          post "/api/v1/users/sign_in", params: {
            user: {
              login: "ldap.user",
              password: "Password!123",
  ...
  test "sign in via ldap works without email attribute using login mapping in single-tenant mode" do
    ...
    authenticator = Struct.new(:result) { def call(login:, password:) = result }.new(auth_result)
    resolver = Struct.new(:user) { def call(email:, login:, name: nil) = user }.new(ldap_user)
