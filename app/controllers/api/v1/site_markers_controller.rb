class Api::V1::SiteMarkersController < Api::V1::BaseController
  ...
  def create
    ...
    marker = Sites::AttachSiteToMap.new(network_map: @network_map, site:, params: marker_params, actor: current_user).call
  ...
  def update
    marker = Sites::UpdateSiteMarker.new(map_node: @marker, params: marker_params, actor: current_user).call
  ...
  def destroy
    Sites::DetachSiteFromMap.new(map_node: @marker).call
    render_data(data: nil)
  end

  def bulk_create
    result = NetworkMaps::BulkAttachSites.new(network_map: @network_map, items: bulk_items, actor: current_user).call
  ...
  def set_marker
    @marker = find_record(@network_map.map_nodes.where(mappable_type: "Site"), params[:id])
  end

  def marker_params
    position = params[:position] || params.dig(:site_marker, :position) || {}
    {
